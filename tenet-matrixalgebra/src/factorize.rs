...
fn compact_svd_plan_cache(
) -> &'static Mutex<lru::LruCache<ValidatedDynamicFusionLayout, Arc<CompactSvdFactorPlan>>> {
    static CACHE: OnceLock<
        Mutex<lru::LruCache<ValidatedDynamicFusionLayout, Arc<CompactSvdFactorPlan>>>,
    > = OnceLock::new();
    CACHE.get_or_init(|| {
    ...
    R: MultiplicityFreeRigidSymbols<Scalar = f64>,
{
    ...
    if let Some(plan) = COMPACT_SVD_PLAN_FRONT.with_borrow(|front| {
        front
            .as_ref()
            .and_then(|(layout, plan)| (layout == &source_layout).then(|| Arc::clone(plan)))
    }) {
        return Ok(Some(plan));
    }
    if let Ok(mut cache) = compact_svd_plan_cache().lock() {
        if let Some(plan) = cache.get(&source_layout) {
            let plan = Arc::clone(plan);
            COMPACT_SVD_PLAN_FRONT.with_borrow_mut(|front| {
                *front = Some((source_layout, Arc::clone(&plan)));
            });
            return Ok(Some(plan));
        }
    }
    ...
    if let Ok(mut cache) = compact_svd_plan_cache().lock() {
        ...
        cache.put(source_layout.clone(), Arc::clone(&built));
    }
    COMPACT_SVD_PLAN_FRONT.with_borrow_mut(|front| {
        *front = Some((source_layout, Arc::clone(&built)));
    });
    Ok(Some(built))
}
