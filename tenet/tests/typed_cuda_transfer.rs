...
fn typed_cuda_direct_execution_matches_host_providers_and_structure() {
    ...
    let product_dual = GradedSpace::try_new_with_shared_provider(
        Arc::clone(&product_rule),
        [
            (product_sector(U1Irrep::new(0), Z2Irrep::EVEN), 2),
            (product_sector(U1Irrep::new(1), Z2Irrep::ODD), 1),
...
fn typed_cuda_reductions_cover_weights_providers_lazy_and_preflight() {
    ...
    let fz2_leg = GradedSpace::try_new_with_shared_provider(
    ...
    let probe_leg =
        GradedSpace::try_new_with_shared_provider(Arc::clone(&probe_provider), [(ProbeSector, 1)])
            .unwrap();
    ...
    let wider = GradedSpace::try_new_with_shared_provider(
        Arc::clone(&u1_provider),
        [(U1Irrep::new(0), 2), (U1Irrep::new(1), 1)],
    )
    .unwrap();
    ...
    let charge0 =
        GradedSpace::try_new_with_shared_provider(Arc::clone(&zn3), [(zn3.irrep(0), 1)]).unwrap();
    let charge1 =
        GradedSpace::try_new_with_shared_provider(Arc::clone(&zn3), [(zn3.irrep(1), 1)]).unwrap();
...
fn typed_cuda_qr_compact_streams_multiplicity_free_f64_factors() {
    ...
    let tall = GradedSpace::try_new_with_shared_provider(
        Arc::clone(&u1),
    ...
    let charge_zero_only =
        GradedSpace::try_new_with_shared_provider(Arc::clone(&u1), [(U1Irrep::new(0), 2)]).unwrap();
    let unmatched =
    ...
    let rank_deficient_leg =
        GradedSpace::try_new_with_shared_provider(Arc::clone(&u1), [(U1Irrep::new(0), 3)]).unwrap();
    ...
    let zn3 = Arc::new(ZNFusionRule::new(3).unwrap());
    let charge0 =
        GradedSpace::try_new_with_shared_provider(Arc::clone(&zn3), [(zn3.irrep(0), 2)]).unwrap();
    let charge1 =
        GradedSpace::try_new_with_shared_provider(Arc::clone(&zn3), [(zn3.irrep(1), 3)]).unwrap();
    let empty: TensorMap<_, f64> =
        TensorMap::from_block_fn(&runtime, [&charge0], [&charge1], |_, _| 1.0).unwrap();
    assert!(empty.data().is_empty());
...
fn typed_cuda_svd_compact_streams_dense_multiplicity_free_f64_factors() {
    ...
    let fermion_leg = GradedSpace::try_new_with_shared_provider(
        Arc::clone(&fermion),
        [(Z2Irrep::EVEN, 2), (Z2Irrep::ODD, 1)],
    )
    .unwrap();
...
fn typed_cuda_svd_compact_handles_large_wide_sector() {
    ...
    let rows =
        GradedSpace::try_new_with_shared_provider(Arc::clone(&provider), [(U1Irrep::new(0), 8)])
            .unwrap();
    let cols =
        GradedSpace::try_new_with_shared_provider(Arc::clone(&provider), [(U1Irrep::new(0), 1025)])
            .unwrap();
...
fn typed_cuda_svd_trunc_matches_host_policies_structure_and_ownership() {
    ...
    let runtime = Runtime::builder().cuda(0).dense_threads(1).build().unwrap();
    let provider = Arc::new(U1FusionRule);
    let rows = GradedSpace::try_new_with_shared_provider(
        Arc::clone(&provider),
        [(U1Irrep::new(0), 3), (U1Irrep::new(1), 2)],
    )
    .unwrap();
    let cols = GradedSpace::try_new_with_shared_provider(
        Arc::clone(&provider),
    ...
    .unwrap();
    let same_identity_space = GradedSpace::try_new_with_shared_provider(
    ...
    let reentrant_leg =
        GradedSpace::try_new_with_shared_provider(Arc::clone(&reentrant), [(ProbeSector, 2)])
            .unwrap();
    ...
    let no_intersection_rows =
        GradedSpace::try_new_with_shared_provider(Arc::clone(&provider), [(U1Irrep::new(4), 2)])
            .unwrap();
...
fn typed_cuda_arithmetic_matches_host_lazy_ownership_and_concurrency() {
    ...
    let other_runtime = Runtime::builder().cuda(0).build().unwrap();
    let other_leg =
        GradedSpace::try_new_with_shared_provider(Arc::clone(&provider), [(U1Irrep::new(0), 1)])
            .unwrap();
...
fn typed_cuda_direct_supports_canonical_lazy_and_rejects_other_scopes_before_mutation() {
    ...
    let provider = Arc::new(U1FusionRule);
    let leg =
        GradedSpace::try_new_with_shared_provider(Arc::clone(&provider), [(U1Irrep::new(0), 2)])
            .unwrap();
    ...
    let zn3_leg =
        GradedSpace::try_new_with_shared_provider(Arc::clone(&zn3), [(zn3.irrep(0), 1)]).unwrap();
    let zn4_leg =
        GradedSpace::try_new_with_shared_provider(Arc::clone(&zn4), [(zn4.irrep(0), 1)]).unwrap();
    ...
    let seam =
        GradedSpace::try_new_with_shared_provider(Arc::clone(&zn3), [(zn3.irrep(1), 1)]).unwrap();
    let right_open =
        GradedSpace::try_new_with_shared_provider(Arc::clone(&zn3), [(zn3.irrep(2), 1)]).unwrap();
