...
fn checked_generic_absorb_validation_precedence_leaves_inputs_unchanged() {
    ...
    let leg = GradedSpace::try_new_with_shared_provider(Arc::clone(&provider), [(vec![1, 1], 2)])
        .unwrap();
    let equal_leg =
        GradedSpace::try_new_with_shared_provider(Arc::clone(&equal_identity), [(vec![1, 1], 2)])
            .unwrap();
    let dual_leg =
        GradedSpace::try_new_with_shared_provider(Arc::clone(&equal_identity), [(vec![1, 1], 2)])
            .and_then(|space| space.try_dual())
            .unwrap();
    let wrong_leg = GradedSpace::try_new_with_shared_provider(
        Arc::clone(&wrong_identity),
        [(vec![1, 0, 1], 2)],
    )
    .unwrap();
