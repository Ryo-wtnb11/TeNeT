...
Scope: public items of the typed facade (`tenet::typed`, `src/typed.rs`) whose
...
A struct's row also covers its documented public fields. A source with no line
range is a deliberate name-only row: the claim has no single defensible
upstream location (the note says why).
...
| `typed::TensorMap::block` | TensorKit | 0.17.0 | `tensors/tensor.jl:447,462-476`, `tensors/adjoint.jl:28`, `tensors/diagonal.jl:153-168` | zero-copy view of the coupled-sector matrix; lazy adjoint `block(parent, c)'`, compact diagonal `Diagonal(view)`. an absent sector is the empty `blockdim(codomain, c) × blockdim(domain, c)` view. Divergence: rows/columns follow TeNeT's storage tree order, reported by `CoupledBlock::row_trees`/`col_trees` |
