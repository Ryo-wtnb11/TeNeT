...
impl TreeTransformCacheStats {
    ...
    /// Source rows reused within successful whole-block plan compilation.
    ///
    /// Rows are not retained across plan misses; exact warm reuse is reported
    /// by [`Self::plan_hits`].
    #[inline]
    ...
    /// Source rows generated by successful whole-block plan compilation.
    #[inline]
    ...
    RuleKey: Clone + Eq + Hash,
{
    ...
    {
        let (plan, row_hits, row_misses) =
            build_tree_pair_transform_group_plan_validated_with_threads(
                source_proof,
                operation,
                self.recoupling_threads,
            )?;
        self.stats.tree_row_hits += row_hits;
        self.stats.tree_row_misses += row_misses;
        Ok(Arc::new(plan))
    }
