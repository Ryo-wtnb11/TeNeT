...
both dtypes, and for compact SVD/EIGH/QR in both dtypes. Full and values-only
factorizations, `eig`, matrix functions,
general
...
| CUDA f64 multiplicity-free | Selected explicit operations, factorizations, and general `tensor!` network execution, intra-operand traces included |
| CUDA c64 multiplicity-free | Transfer, lazy adjoint, arithmetic, reductions, general-axes contraction, canonical compose, general `tensor!` network execution (intra-operand traces included), compact SVD/EIGH (raw device SVD gauge), and compact QR (Host positive-diagonal gauge, #1271) |
