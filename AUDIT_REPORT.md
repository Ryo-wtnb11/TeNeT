...
| CUDA c64 multiplicity-free | Transfer, lazy adjoint, arithmetic, reductions, general-axes contraction, canonical compose, general `tensor!` network execution (intra-operand traces included), and compact SVD/EIGH (raw device SVD gauge); compact QR is a compile-time boundary while tenferro-gpu's `triu` kernel does not compile for `Complex64` |
