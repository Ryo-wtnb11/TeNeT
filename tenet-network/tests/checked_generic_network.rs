...
fn assert_sun_network<D: OracleScalar + Send + Sync + 'static>(n: usize, label: Vec<i64>) {
    ...
    let lhs_leg =
        GradedSpace::try_new_with_shared_provider(Arc::clone(&lhs_provider), [(label.clone(), 1)])
            .unwrap();
    let rhs_leg =
        GradedSpace::try_new_with_shared_provider(Arc::clone(&rhs_provider), [(label.clone(), 1)])
            .unwrap();
    let tail_leg =
        GradedSpace::try_new_with_shared_provider(Arc::clone(&tail_provider), [(label.clone(), 1)])
            .unwrap();
    ...
    let drift_lhs_leg = GradedSpace::try_new_with_shared_provider(
        Arc::clone(&drift_lhs_provider),
        [(label.clone(), 1)],
    )
    .unwrap();
    let drift_middle_leg = GradedSpace::try_new_with_shared_provider(
        Arc::clone(&drift_middle_provider),
        [(label.clone(), 1)],
    )
    .unwrap();
    let drift_tail_leg =
        GradedSpace::try_new_with_shared_provider(Arc::clone(&drift_tail_provider), [(label, 1)])
            .unwrap();
...
fn injected_chain(
    ...
    let leg =
        GradedSpace::try_new_with_shared_provider(Arc::clone(provider), [(vec![1, 1], 1)]).unwrap();
    vec![
...
fn checked_generic_scalar_empty_outer_product_and_single_permute_follow_ordinary_ops() {
    ...
    let empty = GradedSpace::try_new_with_shared_provider(
        Arc::clone(&provider),
