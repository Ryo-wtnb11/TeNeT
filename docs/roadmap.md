...
- [x] 縮約順序 hyperopt: cotengra path backend(hyper/reconfigure/slicing 決定を Python
      cotengra 経由で提供、PR #94)。純 Rust cotengrust 移植は不採用(AGPL、cotengra-python がカバー)
- [ ] sliced 実行の接続(slicing 決定は既存、slice 分割実行の executor が未接続、#93)
...
- [ ] TK export 未 wrap: repartition / catdomain / catcodomain / ishermitian / project_*
