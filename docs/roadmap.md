...
- [ ] 【重要】leg 次元・適合検査が「populated blocks」由来で、疎な対称状態
      (Neel 積状態等)の正当な縮約を拒否する。根本原因: Tensor が Space
      レベルの sector 内容を保持していない。Space 情報の保持 + 検査の
      Space 基準化が必要。エラーメッセージもオペランド名を出すべき
- [ ] tensor! のプラン再利用(現状毎回 greedy 再計画。iTEBD の theta 縮約
      63% のうち一部が planner オーバーヘッド)→ A の plan cache と同件
