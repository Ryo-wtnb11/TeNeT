...
| null/polar factors | PROVED on U1/Z2 fixtures | NEEDS-PROOF | NEEDS-PROOF | NEEDS-PROOF | NEEDS-PROOF | UNSUPPORTED | numerical left/right null proved on provider-neutral checked-Generic fixtures, including SU(3)/SU(4) × f64/c64 cross-μ analytic projectors, exact Arc/lazy-cold ownership, cutoff/zero/disjoint/side-only sectors, and dense/late-provider nonpublication; polar factors remain unsupported |
| EIG/EIGH, `exp`, `inv`, left `solve`, `pinv`, `powi` | PROVED on Z2 fixtures | NEEDS-PROOF | NEEDS-PROOF | NEEDS-PROOF | NEEDS-PROOF | UNSUPPORTED | dense checked-Generic `inv`, global-cutoff `pinv`, Padé-only `exp`, integer `powi`, and left `solve` are proved. `pinv` has non-diagonal f64/c64 tall/wide rectangular Moore-Penrose, invalid-cutoff precedence, strict global-boundary, exact-Arc/lazy-parent, staged SVD/GEMM failure counts, and a SU(3)/SU(4) × f64/c64 μ=2 full-key (2I+N) fixture (selected cross-μ (N), genuinely complex c64) with all four Moore-Penrose identities; TensorKit remains block-local by source audit. Left solve has synthetic preflight/admission counters, direct-into malformed-route coverage, private owned/lazy cache and five-route dense-call checks, backend provenance, and SU(3)/SU(4) × f64/c64 μ=2 full-key/cross-μ oracles; `solve_right` is unsupported. Compact checked-Generic construction remains unsupported. `eig_vals`/`eigh_vals` are proved, while other factor-returning spectra and matrix functions remain UNSUPPORTED |
