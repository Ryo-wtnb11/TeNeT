...
| EIG/EIGH, `exp`, `inv`, left `solve`, `pinv`, `powi` | PROVED on Z2 fixtures | NEEDS-PROOF | NEEDS-PROOF | NEEDS-PROOF | NEEDS-PROOF | UNSUPPORTED | dense checked-Generic `inv`, Padé-only `exp`, and integer `powi` are proved. Checked left `solve` has synthetic coverage but remains NEEDS-PROOF pending its full provider-query, direct-into malformed-route, lazy-cache, and cross-μ oracle evidence; `solve_right` is unsupported. Compact checked-Generic construction remains unsupported. `eig_vals`/`eigh_vals` are proved, while other factor-returning spectra and matrix functions remain UNSUPPORTED |
...
| EIG, null/polar, solve and matrix functions | PROVED on selected Host fixtures | dense `inv`, dense Padé `exp`, integer `powi`, and `eig_vals` proved; left/right solve, null/polar and remaining matrix functions remain unsupported; compact checked-Generic construction remains unsupported | UNSUPPORTED | UNSUPPORTED | UNSUPPORTED | UNSUPPORTED |
