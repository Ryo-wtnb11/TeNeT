...
| EIG/EIGH, `exp`, `inv`, left `solve`, `pinv`, `powi` | PROVED on Z2 fixtures | NEEDS-PROOF | NEEDS-PROOF | NEEDS-PROOF | NEEDS-PROOF | UNSUPPORTED | dense checked-Generic `inv`, Padé-only `exp`, integer `powi`, and left `solve` are proved. `powi(0)` preserves the full admitted block layout without provider queries, and checked behavior is exact-endomorphism/typed-error constrained. Left solve covers synthetic f64/c64 unequal-isomorphic divisors with rectangular RHS and distinct equal-identity provider Arcs, plus SU(3)/SU(4) non-diagonal μ=2 blocks; `solve_right` remains unsupported. Compact checked-Generic construction remains unsupported. `eig_vals`/`eigh_vals` are proved, while other factor-returning spectra and matrix functions remain UNSUPPORTED |
...
| EIG, null/polar, solve and matrix functions | PROVED on selected Host fixtures | dense `inv`, dense Padé `exp`, integer `powi`, `eig_vals`, and left `solve` proved; right solve, null/polar and remaining matrix functions remain unsupported; compact checked-Generic construction remains unsupported | UNSUPPORTED | UNSUPPORTED | UNSUPPORTED | UNSUPPORTED |
...
| Host `exp`, `inv`, left `solve`, `pinv`, `sqrt`, `powi` | checked-Generic `inv` validates isomorphism then admits its swapped output with the source `Arc`; left `solve` validates identity/exact codomain/isomorphism then admits `domain(A) <- domain(B)` with `A`'s exact `Arc`; checked-Generic `exp` validates exact endomorphism and reuses the input bound space; integer `powi` validates endomorphism, uses the admitted identity for zero, and uses one inverse for negative powers | `inv` and left `solve` perform one typed output admission before allocation; `exp` and `powi(0)` perform no provider/coefficient query or new admission; other `powi` exponents inherit checked compose/inv ordering | compact diagonal maps and `solve_right` remain multiplicity-free only; checked-Generic `inv` and left `solve` are dense sector solves, `exp` is dense Padé, and integer `powi` is exponentiation by squaring over compose/inv. Compact checked-Generic construction/preservation remains unsupported | direct `powi(1)` may be an owned body-sharing clone; other direct results are owned outputs. Lazy `powi` finishes as an owned uncached logical result, while lazy inverse and negative powi redirect through `(A^-1)†`; lazy exp and left solve use operation-local materialization; all leave the receiver cache cold |
