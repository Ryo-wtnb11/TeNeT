...
| EIG/EIGH, `exp`, `inv`, `solve`, `pinv`, `powi` | PROVED on Z2 fixtures | NEEDS-PROOF | NEEDS-PROOF | NEEDS-PROOF | NEEDS-PROOF | UNSUPPORTED | dense checked-Generic `inv`, Padé-only `exp`, and integer `powi` are proved on synthetic f64/c64 and SU(3)/SU(4) outer-multiplicity fixtures; `powi(0)` preserves the full admitted block layout without provider queries, and checked behavior is exact-endomorphism/typed-error constrained. Compact checked-Generic construction remains unsupported. `eig_vals`/`eigh_vals` are proved, while other factor-returning spectra and matrix functions remain UNSUPPORTED |
...
| Host `exp`, `inv`, `solve`, `pinv`, `sqrt`, `powi` | checked-Generic `inv` validates isomorphism then admits its swapped output with the source `Arc`; checked-Generic `exp` validates exact endomorphism and reuses the input bound space; integer `powi` validates endomorphism, uses the admitted identity for zero, and uses one inverse for negative powers | `inv` performs its one typed output admission before allocation; `exp` and `powi(0)` perform no provider/coefficient query or new admission; other `powi` exponents inherit checked compose/inv ordering | compact diagonal maps remain multiplicity-free only; checked-Generic `inv` is dense sector solve, `exp` is dense Padé, and integer `powi` is exponentiation by squaring over compose/inv. Compact checked-Generic construction/preservation remains unsupported | direct `powi(1)` may be an owned body-sharing clone; other direct results are owned outputs. Lazy `powi` finishes as an owned uncached logical result, while lazy inverse and negative powi redirect through `(A^-1)†`; all leave the receiver cache cold |
