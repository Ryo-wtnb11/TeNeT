...
| EIG/EIGH, `exp`, `inv`, `solve`, `pinv`, `powi` | PROVED on Z2 fixtures | NEEDS-PROOF | NEEDS-PROOF | NEEDS-PROOF | NEEDS-PROOF | UNSUPPORTED | dense checked-Generic `inv` and Padé-only `exp` are proved on synthetic f64/c64 and SU(3)/SU(4) outer-multiplicity fixtures; compact checked-Generic construction remains unsupported. `eig_vals`/`eigh_vals` are proved, while other factor-returning spectra and matrix functions remain UNSUPPORTED |
...
| Host `exp`, `inv`, `solve`, `pinv`, `sqrt`, `powi` | checked-Generic `inv` validates isomorphism then admits its swapped output with the source `Arc`; checked-Generic `exp` validates exact endomorphism and reuses the input bound space | `inv` performs its one typed output admission before allocation; `exp` performs no provider/coefficient query or new admission | compact diagonal maps remain multiplicity-free only; checked-Generic `inv` is dense sector solve and `exp` is dense Padé. Compact checked-Generic construction/preservation remains unsupported | fresh owned results; lazy inverse redirects through `(A^-1)†`, while lazy exp uses operation-local materialization; both leave the receiver cache cold |
