...
| EIG/EIGH, `exp`, `inv`, `solve`, `pinv`, `powi` | PROVED on Z2 fixtures | NEEDS-PROOF | NEEDS-PROOF | NEEDS-PROOF | NEEDS-PROOF | UNSUPPORTED | `eig_vals`/`eigh_vals` proved on checked-Generic fixtures; factor-returning spectra and matrix functions remain UNSUPPORTED |
