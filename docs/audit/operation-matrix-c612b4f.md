...
| Host `exp`, `inv`, `solve`, `pinv`, `sqrt`, `powi` | checked-Generic `inv` validates isomorphism then admits its swapped output with the source `Arc`; checked-Generic `exp` validates exact endomorphism and reuses the input bound space; `powi` validates endomorphism, uses the admitted identity for zero, and uses one inverse for negative powers | `inv` performs its one typed output admission before allocation; `exp` and `powi(0)` perform no provider/coefficient query or new admission | compact diagonal maps remain multiplicity-free only; checked-Generic `inv` is dense sector solve, `exp` is dense Padé, and `powi` is exponentiation by squaring over compose/inv. Compact checked-Generic construction/preservation remains unsupported | fresh owned results; lazy inverse and negative powi redirect through `(A^-1)†`, while lazy exp uses operation-local materialization; all leave the receiver cache cold |
