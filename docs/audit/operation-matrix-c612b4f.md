...
| EIG/EIGH, `exp`, `inv`, `solve`, `pinv`, `powi` | PROVED on Z2 fixtures | NEEDS-PROOF | NEEDS-PROOF | NEEDS-PROOF | NEEDS-PROOF | UNSUPPORTED | dense `inv` proved on checked-Generic SU(3)/SU(4) f64/c64 fixtures; `eig_vals`/`eigh_vals` are proved, while other factor-returning spectra and matrix functions remain UNSUPPORTED; compact checked-Generic construction remains unsupported |
...
| EIG, null/polar, solve and matrix functions | PROVED on selected Host fixtures | dense `inv` and `eig_vals` proved; null/polar/solve and remaining matrix functions remain unsupported; compact checked-Generic construction remains unsupported | UNSUPPORTED | UNSUPPORTED | UNSUPPORTED | UNSUPPORTED |
...
| Host `exp`, `inv`, `solve`, `pinv`, `sqrt`, `powi` | validates square/compatible spaces and reuses or derives the admitted output space | checked-Generic `inv` admits its swapped output through `TypedTensorRootDispatch` with the source `Arc` before allocation; no F/R/CGC work occurs in the shared inverse body | compact diagonal maps when legal for multiplicity-free inputs; checked Generic `inv` is dense-only and reuses the coefficient-free sector solve. Checked-Generic compact construction/preservation remains unsupported | fresh owned result; the checked-Generic lazy inverse redirects through `(A^-1)†` and leaves the receiver cache cold |
