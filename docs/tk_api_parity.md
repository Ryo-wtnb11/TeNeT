...
| `otimes` (`⊗`, tensor) | has | `Tensor::otimes` / `typed::TensorMap::otimes` | Tensor product in one category (`linalg.jl:556`), with exact external order `cod(A), cod(B); dom(A), dom(B)`. It uses the checked fusion contraction route, not a dense Kronecker loop. |
