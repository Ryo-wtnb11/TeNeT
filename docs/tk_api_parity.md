...
| `exp` | has | `Tensor::exp` / `TensorMap::exp` | Any endomorphism, as in TK (`exp!`, `linalg.jl:420-427`, which checks only `domain == codomain`). Dense input dispatches on the blocks: Hermitian blocks keep the exact spectral route `v exp(d) v^H`, everything else takes blockwise scaling-and-squaring Padé [13/13] (Higham 2005) — the algorithm behind the `LinearAlgebra.exp!` TK calls — at `O(Σ_c n_c³)` with `O(max_c n_c²)` reused scratch (#577). Values therefore agree with TK to approximant error, not bitwise: TK drops to Padé degree 3/5/7/9 below `‖A‖₁ = 2.1` where TeNeT always uses [13/13]. Compact diagonal storage takes TK's `exp(::DiagonalTensorMap)` (`diagonal.jl:383-390`) instead: elementwise on the stored values, `O(Σ_c k_c)`, staying compact and — as upstream — with no hermiticity gate (#576 typed, #578 erased). |
