...
| `otimes` (`⊗`, tensor) | has-generic-erased-SU3 | `Tensor::otimes` / `typed::TensorMap::otimes` | Tensor product in one category (`linalg.jl:556`), with exact external order `cod(A), cod(B); dom(A), dom(B)`. Codomain and domain trees merge independently through F moves with one shared root outer-multiplicity label; no legs cross, no R symbol is evaluated, and no dense Kronecker temporary is built. The erased SU(3) lane is supported with real F coefficients. Typed Generic admission, checked/fallible Generic executor admission, and complex categorical F coefficients remain separate work. |
