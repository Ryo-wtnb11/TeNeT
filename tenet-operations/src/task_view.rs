...
mod allocation_tests {
    ...
    #[test]
    fn warmed_task_view_admission_is_allocation_free() {
        let structure = Arc::new(BlockStructure::packed_column_major(1, [vec![4]]).unwrap());
        let transform = TreeTransformStructure::compile_structures(
            &structure,
            &structure,
            &[TreeTransformBlockSpec::single(0, 0, 1.0_f64)],
        ...
        let admit = || {
            ...
            task.validate_structures_and_lengths(&structure, &structure, 4, 4)
                .unwrap();
            task.validate_workspace_requirements::<f64>().unwrap();
            black_box(task.workspace_requirements());
        };
        admit();
