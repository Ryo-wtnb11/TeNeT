...
The measurement ran on an unchanged cache. The recommendation at the end is
implemented by the commit that adds this note to `main`; see
[Implemented bounds](#implemented-bounds).
...
Replay offline from the committed trace (this also runs the new-bounds gate
and exits non-zero on a violation):

```text
cd benchmarks/history
python3 ../complete_structure_census.py \
  complete-structure-census-2026-09-24-steps.csv.gz \
  complete-structure-census-2026-09-24-trace.txt.gz
```

Re-trace from scratch. The census example and the lldb tracer are
measurement-only and stay on branch `census-1365` at `334ee1be`; check that
commit out to run:
...
2. **Shrink `MAX_ENTRY_BYTES` to budget / 16 = 256 KiB** (262_144). That is
   ...
   possible at today's 93.6 %. *Not adopted; see
   [Implemented bounds](#implemented-bounds).*
...
`CompleteHomSpaceStructureCache` in `tenet-core/src/fusion_space.rs` now has a
byte budget of 4 MiB (binding) and an entry cap of 1024 (backstop).
`max_entry_bytes` keeps its previous value, 1_650_641, which is deliberately
not shrunk (recommendation 2 above). A limit of 256 KiB would bypass
high-rank structures that the previous bounds cache. One example is the rank-7
U(1) destination in `tenet-tensors/tests/warm_contract_compile_allocations.rs`,
which is charged 361,285 bytes; with the shrunk limit, that test's warm compile
went from 19 to 109 allocations. PEPS and other high-rank shapes may be larger
still, so any smaller limit only moves the cliff. The 4 MiB budget is what
removes the monopolisation: the largest admissible entry is now about 39 % of
the budget, not 93.6 %, so one outlier can no longer evict everything and hold
the budget alone. Eviction stays FIFO: hits `peek`
under the read lock. The type docs now say that the `lru::LruCache` serves only
as an insertion-ordered map. The map is no longer built with the cap as its
capacity, so the hash table does not preallocate 1024 slots that the budget
does not charge.

Offline replay of the committed trace under these bounds
(`complete_structure_census.py`, section "Replay under the new bounds"):

| workload | warm lookups | warm misses | compulsory | warm evictions | evictions | bypasses | peak entries | peak bytes |
|---|---:|---:|---:|---:|---:|---:|---:|---:|
| mps/U1/L6 | 268 | 2 | 2 | 0 | 0 | 0 | 49 | 277848 |
| mps/SU2/L6 | 260 | 1 | 1 | 0 | 0 | 0 | 45 | 199601 |
| mps/U1/L24 | 1348 | 3 | 3 | 0 | 0 | 0 | 59 | 353656 |
| mps/SU2/L24 | 1332 | 17 | 17 | 0 | 0 | 0 | 90 | 422474 |
| itebd/U1 | 900 | 0 | 0 | 0 | 0 | 0 | 73 | 620518 |
| e1 (360 rows, summed) | — | 0 | 0 | 0 | 0 | 0 | 7 (max) | 100984 (max) |
| conj (60 rows, summed) | — | 0 | 0 | 0 | 0 | 0 | 6 (max) | 100984 (max) |

Every warm miss is compulsory, nothing is evicted or bypassed, and the largest
retained set, 620 KB, is 15 % of the budget. Peak bytes here count every key a
workload ever admitted, which is more than the live set of section 2.
`tenet/tests/complete_structure_working_set.rs` checks the same property on
the production cache: an E1-style ledger and an 8-site sweep-like loop give
zero warm misses, admissions and evictions, with exact hit counts. At the old
cap of 5 the sweep loop misses and evicts on all 21 warm lookups of each pass.
