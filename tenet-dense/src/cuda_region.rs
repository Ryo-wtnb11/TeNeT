...
/// (`tenferro-gpu-0.5.0/src/cubecl/permutation.rs:723`) folds a view's element
/// offset into the operand pointer but still reports the allocation's
/// alignment, and `copy_view_into` (`permutation.rs:480`) passes that number
/// straight into the descriptor. cuTENSOR selects a vectorized kernel from it,
/// so an operand that starts mid-allocation faults the launch with
/// `cudaErrorMisalignedAddress`. Tenferro main fixed this in `25379dd`
/// (tensor4all/tenferro-rs#1836, `device_address_alignment`); the pinned 0.5.0
/// release did not get it.
...
///
/// The base allocation satisfies 256 bytes — that is what `cudaMalloc`
/// guarantees and what Tenferro's own offset-0 operands already rely on — so
/// the promise survives the shift exactly when the byte offset is a multiple
/// of 256. This is the descriptor contract itself, not the kernel heuristic
